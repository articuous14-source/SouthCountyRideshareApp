...
function switchView(view) {
    ...
    // Hide unconfirmed rides view if switching to another view
    const unconfirmedView = document.getElementById('unconfirmedRidesView');
    if (unconfirmedView) {
        unconfirmedView.style.display = 'none';
    }
    ...
    if (view === 'overview') {
        ...
        if (!currentCustomerFilter) {
            currentCustomerFilter = 'active';
        }
        // Update button states
        document.querySelectorAll('#archiveView .view-btn').forEach(btn => {
            btn.classList.remove('active');
        });
        const btnId = currentCustomerFilter === 'active' ? 'btn-active-customers' : 'btn-blocked-customers';
...
// Expose switchView to global scope
window.switchView = switchView;

// Check for rides that need confirmation (admin view - shows all drivers)
// View unconfirmed rides (admin - grouped by driver)
function viewUnconfirmedRidesAdmin() {
    console.log('viewUnconfirmedRidesAdmin called');
    
    // Hide all other views
    document.querySelectorAll('.list-view, .calendar-view').forEach(view => {
        view.style.display = 'none';
    });
    
    // Show unconfirmed rides view
    const unconfirmedView = document.getElementById('unconfirmedRidesView');
    console.log('unconfirmedRidesView element:', unconfirmedView);
    
    if (unconfirmedView) {
        unconfirmedView.style.display = 'block';
        currentView = 'unconfirmed-rides';
        
        // Load unconfirmed rides grouped by driver
        displayUnconfirmedRidesByDriver();
        
        // Clear active state from all view buttons
        document.querySelectorAll('.view-btn').forEach(btn => {
            btn.classList.remove('active');
        });
    } else {
        console.error('unconfirmedRidesView element not found');
    }
}

// Expose to global scope for onclick access
window.viewUnconfirmedRidesAdmin = viewUnconfirmedRidesAdmin;

// Display unconfirmed rides grouped by driver
function displayUnconfirmedRidesByDriver() {
    const container = document.getElementById('unconfirmedRidesList');
    if (!container) return;
    ...
    if (unconfirmedRides.length === 0) {
        container.innerHTML = `
            <div class="empty-state">
                <h3>No Rides Need Confirmation</h3>
                <p>All accepted rides within the next 48 hours have been confirmed.</p>
            </div>
        `;
        return;
    }
    ...
    unconfirmedRides.forEach(ride => {
        ...
        const driverName = ride.driverName || 'Unassigned';
        ...
        ridesByDriver[driverId].rides.push(ride);
    });
    
    // Sort rides within each driver group by date/time
    Object.values(ridesByDriver).forEach(driverGroup => {
        driverGroup.rides.sort((a, b) => {
    ...
    container.innerHTML = Object.entries(ridesByDriver).map(([driverId, driverGroup]) => {
        const rideCount = driverGroup.rides.length;
        return `
            <div style="background: white; border-radius: 10px; padding: 1.5rem; margin-bottom: 1.5rem; box-shadow: 0 2px 8px rgba(0,0,0,0.1); border-left: 5px solid #ff6b6b;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; padding-bottom: 1rem; border-bottom: 2px solid #f0f0f0;">
                    <h3 style="color: var(--primary-color); margin: 0; font-size: 1.3rem;">
                        👤 ${driverGroup.name}
                    </h3>
                    <span style="background: #ff6b6b; color: white; padding: 0.5rem 1rem; border-radius: 20px; font-weight: 600; font-size: 0.9rem;">
                        ${rideCount} ride${rideCount > 1 ? 's' : ''} need${rideCount === 1 ? 's' : ''} confirmation
                    </span>
                </div>
                <div style="display: flex; flex-direction: column; gap: 1rem;">
                    ${driverGroup.rides.map(ride => createUnconfirmedRideCard(ride)).join('')}
                </div>
            </div>
        `;
    }).join('');
...
// Create a card for an unconfirmed ride
function createUnconfirmedRideCard(ride) {
    ...
    return `
        <div style="background: #f8f9fa; border-radius: 8px; padding: 1rem; border: 2px solid #e0e0e0;">
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin-bottom: 0.75rem;">
                <div>
                    <span style="color: #666; font-size: 0.85rem; display: block;">Route</span>
                    <span style="font-weight: 600; color: #333;">${ride.pickup} → ${ride.destination}</span>
                </div>
                <div>
                    <span style="color: #666; font-size: 0.85rem; display: block;">Date & Time</span>
                    <span style="font-weight: 600; color: #333;">${formattedDate} at ${formattedTime}</span>
                </div>
                <div>
                    <span style="color: #666; font-size: 0.85rem; display: block;">Passenger</span>
                    <span style="font-weight: 600; color: #333;">${ride.name}</span>
                </div>
            </div>
            <button onclick="viewRideDetails('${ride.id}')" style="background: var(--primary-color); color: white; border: none; padding: 0.5rem 1rem; border-radius: 5px; cursor: pointer; font-weight: 600; font-size: 0.9rem;">
                View Details
            </button>
        </div>
    `;
...
function calculateTotalCommission() {
    ...
    drivers.forEach(driver => {
        ...
        totalCommission += driverIncome * 0.15;
    });
...
function calculateExpectedConfirmedCommission() {
    ...
    drivers.forEach(driver => {
        ...
        expectedRides.forEach(ride => {
            ...
            expectedCommission += pricing.totalPrice * 0.15;
        });
        ...
        confirmedRides.forEach(ride => {
            ...
            confirmedCommission += pricing.totalPrice * 0.15;
        });
    });
...
function loadQuickIncome() {
    ...
    drivers.forEach(driver => {
        ...
        driverRides.forEach(ride => {
            ...
            if (ride.status === 'accepted') {
                weeklyExpectedCommission += pricing.totalPrice * 0.15;
            } else if (ride.status === 'completed') {
                weeklyConfirmedCommission += pricing.totalPrice * 0.15;
            }
        });
    });
    ...
    drivers.forEach(driver => {
        ...
        driverRides.forEach(ride => {
            ...
            if (ride.status === 'accepted') {
                monthlyExpectedCommission += pricing.totalPrice * 0.15;
            } else if (ride.status === 'completed') {
                monthlyConfirmedCommission += pricing.totalPrice * 0.15;
            }
        });
    });
...
function loadRides() {
    ...
    // Populate month filter if in calendar view
    if (currentView === 'calendar') {
        populateMonthFilter(rides);
...
function displayListView(rides) {
    ...
    container.innerHTML = sortedWeeks.map((weekKey, weekIndex) => {
        ...
        const cancelledCount = weekData.rides.filter(r => r.status === 'cancelled').length;
...
function loadDrivers() {
    ...
    driversList.innerHTML = drivers.map(driver => {
        ...
        // Calculate 15% commission for this driver
        const commission = driverIncome * 0.15;
        ...
        const expectedServiceFee = expectedIncome * 0.15;
        const confirmedServiceFee = confirmedIncome * 0.15;
        ...
        return `
            <div class="driver-card" style="cursor: pointer;" onclick="showDriverDetails('${driver.username}')">
                ...
                <div class="driver-info-item" style="margin-top: 1rem; padding-top: 1rem; border-top: 1px solid var(--light-bg); background: #fff3cd; padding: 1rem; border-radius: 5px;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem;">
                        <strong style="color: #856404;">Expected Service Fee (15%):</strong>
                        <strong style="color: #856404; font-size: 1.1rem;">${formatPrice(expectedServiceFee)}</strong>
                    </div>
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem;">
                        <strong style="color: #155724;">Confirmed Service Fee (15%):</strong>
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center; padding-top: 0.5rem; border-top: 1px solid #ffc107;">
                        <strong style="color: #856404;">Total Service Fee (15%):</strong>
...
function showDriverDetails(driverUsername) {
    ...
    expectedRides.forEach(ride => {
        ...
        const acceptedDate = ride.acceptedAt ? new Date(ride.acceptedAt).toLocaleDateString('en-US', { 
            month: 'short', 
            day: 'numeric', 
            year: 'numeric',
            hour: 'numeric',
        ...
        <div style="margin-bottom: 2rem;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1rem;">
                <div style="background: #fff3cd; padding: 1.5rem; border-radius: 5px; border: 2px solid #ffc107;">
                    <div style="font-weight: 700; color: #856404; margin-bottom: 0.5rem;">Expected Service Fee (15%)</div>
                ...
                <div style="background: #d4edda; padding: 1.5rem; border-radius: 5px; border: 2px solid #28a745;">
                    <div style="font-weight: 700; color: #155724; margin-bottom: 0.5rem;">Confirmed Service Fee (15%)</div>
                ...
                <div style="background: #fff3cd; padding: 1.5rem; border-radius: 5px; border: 2px solid #ff9800;">
                    <div style="font-weight: 700; color: #856404; margin-bottom: 0.5rem;">Total Service Fee (15%)</div>
...
function loadIncome() {
    ...
    drivers.forEach(driver => {
        ...
        driverRides.forEach(ride => {
            ...
            if (ride.status === 'accepted') {
                allTimeExpectedCommission += pricing.totalPrice * 0.15;
            } else if (ride.status === 'completed') {
                allTimeConfirmedCommission += pricing.totalPrice * 0.15;
            }
        });
    });
        ...
        <div class="income-period">
            ...
            <div class="income-breakdown">
                ...
                <div class="income-item" style="background: #fff3cd; border: 2px solid #ffc107;">
                    <div class="income-item-label" style="color: #856404; font-weight: 700;">Expected Service Fee (15%)</div>
        ...
        <div class="income-period" style="margin-top: 2rem; padding-top: 2rem; border-top: 3px solid var(--primary-color);">
            ...
            <div class="income-breakdown">
                <div class="income-item" style="background: #fff3cd; border: 3px solid #ffc107; padding: 1.5rem;">
                    <div class="income-item-label" style="color: #856404; font-weight: 700; font-size: 1.1rem;">Expected Service Fee from All Drivers (15%)</div>
                ...
                <div class="income-item" style="background: #d4edda; border: 3px solid #28a745; padding: 1.5rem;">
                    <div class="income-item-label" style="color: #155724; font-weight: 700; font-size: 1.1rem;">Confirmed Service Fee from All Drivers (15%)</div>
...
let currentCustomerFilter = 'active';

// Switch customer filter (active/blocked)
function switchCustomerFilter(filter) {
    ...
    const btnId = filter === 'active' ? 'btn-active-customers' : 'btn-blocked-customers';
    const btn = document.getElementById(btnId);
    if (btn) btn.classList.add('active');
...
function loadArchive() {
    const customers = getCustomerArchive();
    const container = document.getElementById('archiveContent');
    
    // Filter by active/blocked
    const filteredCustomers = customers.filter(c => {
        if (currentCustomerFilter === 'blocked') {
    ...
    if (filteredCustomers.length === 0) {
        const message = currentCustomerFilter === 'blocked' 
            ? 'No blocked customers.'
            : 'Customer information will automatically be added when ride requests are submitted.';
        container.innerHTML = `
            <div class="empty-state">
                <h3>No Customers</h3>
                <p>${message}</p>
    ...
    // Sort by ride count (descending), then by name
    ...
    container.innerHTML = `
        <div style="background: white; border-radius: 10px; padding: 1.5rem; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
            <div style="margin-bottom: 1rem; padding-bottom: 1rem; border-bottom: 2px solid #e0e0e0;">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <h3 style="color: var(--primary-color); margin: 0;">${currentCustomerFilter === 'blocked' ? 'Blocked' : 'Active'} Customers: ${filteredCustomers.length}</h3>
                    <div style="color: #666; font-size: 0.9rem;">
                        Total Rides: ${filteredCustomers.reduce((sum, c) => sum + (c.rideCount || 1), 0)}
                    </div>
                </div>
            </div>
            <div style="overflow-x: auto;">
                <table style="width: 100%; border-collapse: collapse;">
                    <thead>
                        <tr style="background: var(--light-bg);">
                            <th style="padding: 1rem; text-align: left; border-bottom: 2px solid var(--primary-color); color: var(--primary-color); font-weight: 600;">Name</th>
                            <th style="padding: 1rem; text-align: left; border-bottom: 2px solid var(--primary-color); color: var(--primary-color); font-weight: 600;">Email</th>
                            <th style="padding: 1rem; text-align: left; border-bottom: 2px solid var(--primary-color); color: var(--primary-color); font-weight: 600;">Phone</th>
                            <th style="padding: 1rem; text-align: left; border-bottom: 2px solid var(--primary-color); color: var(--primary-color); font-weight: 600;">Home Address</th>
                            <th style="padding: 1rem; text-align: center; border-bottom: 2px solid var(--primary-color); color: var(--primary-color); font-weight: 600;">Ride Count</th>
                            <th style="padding: 1rem; text-align: left; border-bottom: 2px solid var(--primary-color); color: var(--primary-color); font-weight: 600;">First Ride</th>
                            <th style="padding: 1rem; text-align: left; border-bottom: 2px solid var(--primary-color); color: var(--primary-color); font-weight: 600;">Last Ride</th>
                            <th style="padding: 1rem; text-align: center; border-bottom: 2px solid var(--primary-color); color: var(--primary-color); font-weight: 600;">Actions</th>
                        </tr>
                    </thead>
                    <tbody id="customerTableBody">
                        ${sortedCustomers.map(customer => createCustomerRow(customer)).join('')}
                    </tbody>
                </table>
            </div>
        </div>
...
function createCustomerRow(customer) {
    const homeAddress = customer.homeAddress ? 
        `${customer.homeAddress.street || ''}, ${customer.homeAddress.city || ''}, ${customer.homeAddress.state || ''} ${customer.homeAddress.zip || ''}`.replace(/^,\s*|,\s*$/g, '').trim() : 
        'N/A';
    
    const firstRideDate = customer.firstRideDate ? 
        new Date(customer.firstRideDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 
        'N/A';
    
    const lastRideDate = customer.lastRideDate ? 
        new Date(customer.lastRideDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 
        'N/A';
    
    const rideCountBadge = customer.rideCount > 1 ? 
        `<span style="background: var(--accent-color); color: white; padding: 0.25rem 0.75rem; border-radius: 20px; font-weight: 600; font-size: 0.9rem;">${customer.rideCount} rides</span>` :
        `<span style="background: #e0e0e0; color: #666; padding: 0.25rem 0.75rem; border-radius: 20px; font-size: 0.9rem;">1 ride</span>`;
    
    const blockedBadge = customer.blocked ? 
        `<span style="background: #dc3545; color: white; padding: 0.25rem 0.75rem; border-radius: 20px; font-weight: 600; font-size: 0.85rem; margin-left: 0.5rem;">BLOCKED</span>` : '';
    
    const actionButtons = currentCustomerFilter === 'blocked' ? 
        `<button onclick="unblockCustomer('${customer.id}')" style="background: #28a745; color: white; border: none; padding: 0.5rem 1rem; border-radius: 5px; cursor: pointer; font-size: 0.85rem; margin-right: 0.5rem;">Unblock</button>
         <button onclick="deleteCustomer('${customer.id}')" style="background: #dc3545; color: white; border: none; padding: 0.5rem 1rem; border-radius: 5px; cursor: pointer; font-size: 0.85rem;">Delete</button>` :
        `<button onclick="blockCustomer('${customer.id}')" style="background: #ffc107; color: #333; border: none; padding: 0.5rem 1rem; border-radius: 5px; cursor: pointer; font-size: 0.85rem; margin-right: 0.5rem;">Block</button>
         <button onclick="deleteCustomer('${customer.id}')" style="background: #dc3545; color: white; border: none; padding: 0.5rem 1rem; border-radius: 5px; cursor: pointer; font-size: 0.85rem;">Delete</button>`;
    
    return `
        <tr style="border-bottom: 1px solid #e0e0e0; ${customer.blocked ? 'background: #fff3cd;' : ''}" data-customer-name="${(customer.name || '').toLowerCase()}" data-customer-email="${(customer.email || '').toLowerCase()}" data-customer-phone="${customer.phone || ''}">
            <td style="padding: 1rem;">
                <span onclick="showCustomerDetails('${customer.id}')" style="font-weight: 600; color: var(--primary-color); cursor: pointer; text-decoration: underline;">${customer.name || 'N/A'}</span>
                ${blockedBadge}
            </td>
            <td style="padding: 1rem; color: #666;">${customer.email || 'N/A'}</td>
            <td style="padding: 1rem; color: #666;">${customer.phone || 'N/A'}</td>
            <td style="padding: 1rem; color: #666; font-size: 0.9rem;">${homeAddress}</td>
            <td style="padding: 1rem; text-align: center;">${rideCountBadge}</td>
            <td style="padding: 1rem; color: #666; font-size: 0.9rem;">${firstRideDate}</td>
            <td style="padding: 1rem; color: #666; font-size: 0.9rem;">${lastRideDate}</td>
...
function exportCustomers() {
    ...
    // Create CSV header
    let csv = 'Name,Email,Phone,Home Address,Ride Count,First Ride,Last Ride\n';
    
    // Sort by ride count (descending)
    const sortedCustomers = [...customers].sort((a, b) => b.rideCount - a.rideCount);
    
    // Add customer data
    sortedCustomers.forEach(customer => {
        const homeAddress = customer.homeAddress ? 
            `"${customer.homeAddress.street || ''}, ${customer.homeAddress.city || ''}, ${customer.homeAddress.state || ''} ${customer.homeAddress.zip || ''}"` : 
            '';
        
        const firstRide = customer.firstRideDate || '';
        const lastRide = customer.lastRideDate || '';
        
        csv += `"${customer.name || ''}","${customer.email || ''}","${customer.phone || ''}",${homeAddress},${customer.rideCount || 1},${firstRide},${lastRide}\n`;
    });
...
// Import existing rides into customer archive
function importRidesToArchive() {
    if (typeof importRidesToCustomerArchive === 'function') {
        const result = importRidesToCustomerArchive();
        alert(`Import complete!\n\nTotal rides processed: ${result.total}\nNew customers added: ${result.imported}\nExisting customers updated: ${result.updated}\n\nPlease refresh the archive view to see the updated data.`);
        // Reload archive if we're on that view
...
// Store current customer ID for ride creation
let currentCustomerForRide = null;
...
function showCustomerDetails(customerId) {
    ...
    // Store customer for ride creation
    currentCustomerForRide = customer;
    
    // Get all rides for this customer
    const rides = getRides();
    const customerRides = rides.filter(ride => {
        const normalizedName = (ride.name || '').replace(/\s+/g, '').toLowerCase().trim();
        const normalizedEmail = (ride.email || '').replace(/\s+/g, '').toLowerCase().trim();
        const normalizedPhone = (ride.phone || '').replace(/\D/g, '');
        
        const customerName = (customer.name || '').replace(/\s+/g, '').toLowerCase().trim();
        const customerEmail = (customer.email || '').replace(/\s+/g, '').toLowerCase().trim();
    ...
    let totalRevenue = 0;
    const rates = typeof getRates === 'function' ? getRates() : (typeof ratesData !== 'undefined' ? ratesData : {});
    ...
    customerRides.forEach(ride => {
        if (ride.status === 'completed' || ride.status === 'accepted') {
            ...
            } else if (typeof calculatePricing === 'function') {
                pricing = calculatePricing(ride, rates);
            } else {
                // Fallback: use recalculated price if available, otherwise 0
                pricing = {
                    totalPrice: ride.recalculatedPrice || 0
                };
            }
            totalRevenue += pricing.totalPrice || 0;
        }
    });
    ...
    const firstRideDate = customer.firstRideDate ? 
        new Date(customer.firstRideDate).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' }) : 
        'N/A';
    
    const lastRideDate = customer.lastRideDate ? 
        new Date(customer.lastRideDate).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' }) : 
        'N/A';
    
    const blockedStatus = customer.blocked ? 
        '<span style="background: #dc3545; color: white; padding: 0.5rem 1rem; border-radius: 5px; font-weight: 600;">BLOCKED</span>' :
        '<span style="background: #28a745; color: white; padding: 0.5rem 1rem; border-radius: 5px; font-weight: 600;">Active</span>';
    ...
    content.innerHTML = `
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem; margin-bottom: 2rem;">
            <div>
                <h3 style="color: var(--primary-color); margin-bottom: 1rem; border-bottom: 2px solid #e0e0e0; padding-bottom: 0.5rem;">Contact Information</h3>
                ...
                <div style="margin-bottom: 1rem;">
                    ...
                    <span style="color: #666;">${homeAddress}</span>
                </div>
                <div style="margin-top: 1rem;">
                    <strong>Status:</strong> ${blockedStatus}
                </div>
            </div>
            <div>
                <h3 style="color: var(--primary-color); margin-bottom: 1rem; border-bottom: 2px solid #e0e0e0; padding-bottom: 0.5rem;">Ride Statistics</h3>
                <div style="margin-bottom: 1rem;">
                    <strong>Total Rides:</strong> ${customer.rideCount || 0}
                </div>
                <div style="margin-bottom: 1rem;">
                    <strong>First Ride:</strong> ${firstRideDate}
                </div>
                <div style="margin-bottom: 1rem;">
                    <strong>Last Ride:</strong> ${lastRideDate}
                </div>
                <div style="margin-top: 1.5rem; padding: 1rem; background: #e3f2fd; border-radius: 5px;">
                    <strong style="font-size: 1.1rem; color: var(--primary-color);">Total Revenue:</strong>
                    <div style="font-size: 1.5rem; font-weight: 700; color: var(--accent-color); margin-top: 0.5rem;">
        ...
        <div style="margin-top: 2rem; padding-top: 2rem; border-top: 2px solid #e0e0e0;">
            <h3 style="color: var(--primary-color); margin-bottom: 1rem;">Ride History (${customerRides.length} rides)</h3>
            <div style="max-height: 400px; overflow-y: auto;">
                ${customerRides.length === 0 ? '<p style="color: #666;">No ride history available.</p>' : 
                customerRides.map(ride => {
                    const dateParts = ride.date.split('-');
                    const dateObj = new Date(parseInt(dateParts[0]), parseInt(dateParts[1]) - 1, parseInt(dateParts[2]));
                    const formattedDate = dateObj.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
                    const formattedTime = new Date(`2000-01-01T${ride.time}`).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true });
                    ...
                    const statusBadge = ride.status === 'completed' ? 
                        '<span style="background: #28a745; color: white; padding: 0.25rem 0.5rem; border-radius: 3px; font-size: 0.85rem;">Completed</span>' :
                        ride.status === 'accepted' ?
                        '<span style="background: #17a2b8; color: white; padding: 0.25rem 0.5rem; border-radius: 3px; font-size: 0.85rem;">Accepted</span>' :
                        '<span style="background: #ffc107; color: #333; padding: 0.25rem 0.5rem; border-radius: 3px; font-size: 0.85rem;">Pending</span>';
                    
                    return `
                        <div style="background: #f8f9fa; padding: 1rem; border-radius: 5px; margin-bottom: 0.75rem; border-left: 4px solid var(--primary-color);">
                            ...
                            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem; margin-top: 0.75rem; font-size: 0.9rem;">
                                <div><strong>Vehicle:</strong> ${ride.vehicleType || 'N/A'}</div>
                                <div><strong>Passengers:</strong> ${ride.passengers || 'N/A'}</div>
                                <div><strong>Amount:</strong> <span style="color: var(--accent-color); font-weight: 600;">${formatPrice(pricing.totalPrice || 0)}</span></div>
                            </div>
                        </div>
                    `;
                }).join('')}
...
function closeCustomerDetailsModal(keepCustomerData = false) {
    const modal = document.getElementById('customerDetailsModal');
    if (modal) {
        modal.style.display = 'none';
    }
    // Only clear customer data if we're not about to use it for creating a ride
    if (!keepCustomerData) {
        currentCustomerForRide = null;
    }
}

// Create ride for customer
function createRideForCustomer() {
    console.log('createRideForCustomer called');
    console.log('currentCustomerForRide:', currentCustomerForRide);
    
    if (!currentCustomerForRide) {
        alert('Customer information not available. Please click on a customer name first.');
        return;
    }
    
    // Store customer data before closing modal (since closeCustomerDetailsModal clears it)
    const customerData = { ...currentCustomerForRide };
    
    // Open create ride modal first (before closing customer details)
    const modal = document.getElementById('createRideModal');
    console.log('Modal element:', modal);
    
    if (!modal) {
        alert('Create ride form not found. Please refresh the page and try again.');
        console.error('createRideModal element not found in DOM');
        return;
    }
    
    // Close customer details modal (but keep customer data for now)
    closeCustomerDetailsModal(true);
    
    // Pre-fill customer information
    const nameField = document.getElementById('createRideName');
    const emailField = document.getElementById('createRideEmail');
    const phoneField = document.getElementById('createRidePhone');
    
    if (!nameField || !emailField || !phoneField) {
        alert('Form fields not found. Please refresh the page and try again.');
        console.error('Form fields not found:', { nameField, emailField, phoneField });
        return;
    }
    ...
    if (customerData.homeAddress) {
        const addr = customerData.homeAddress;
        const pickupStreet = document.getElementById('createRidePickupStreet');
        const pickupCity = document.getElementById('createRidePickupCity');
        const pickupState = document.getElementById('createRidePickupState');
        const pickupZip = document.getElementById('createRidePickupZip');
        const destStreet = document.getElementById('createRideDestinationStreet');
        const destCity = document.getElementById('createRideDestinationCity');
        const destState = document.getElementById('createRideDestinationState');
        const destZip = document.getElementById('createRideDestinationZip');
        
        if (pickupStreet) pickupStreet.value = addr.street || '';
        if (pickupCity) pickupCity.value = addr.city || '';
        if (pickupState) pickupState.value = addr.state || '';
        if (pickupZip) pickupZip.value = addr.zip || '';
        if (destStreet) destStreet.value = addr.street || '';
        if (destCity) destCity.value = addr.city || '';
        if (destState) destState.value = addr.state || '';
        if (destZip) destZip.value = addr.zip || '';
    }
    
    // Set default date to today
    const today = new Date();
    const dateStr = today.toISOString().split('T')[0];
    const dateField = document.getElementById('createRideDate');
    if (dateField) {
    ...
    const timeField = document.getElementById('createRideTime');
    if (timeField) {
    ...
    // Clear customer data after a short delay to allow form to populate
    setTimeout(() => {
        currentCustomerForRide = null;
    }, 100);
}
...
// Handle location change for create ride form
function handleCreateRideLocationChange(type) {
    const pickupSelect = document.getElementById('createRidePickup');
    const destinationSelect = document.getElementById('createRideDestination');
    const pickupHomeAddress = document.getElementById('createRidePickupHomeAddress');
    const destinationHomeAddress = document.getElementById('createRideDestinationHomeAddress');
    const flightInfo = document.getElementById('createRideFlightInfo');
    const flightNumber = document.getElementById('createRideFlightNumber');
    const airline = document.getElementById('createRideAirline');
    
    // Show/hide pickup home address
    if (pickupSelect.value === 'Home') {
        pickupHomeAddress.style.display = 'block';
    } else {
        pickupHomeAddress.style.display = 'none';
    }
    
    // Show/hide destination home address
    if (destinationSelect.value === 'Home') {
        destinationHomeAddress.style.display = 'block';
    } else {
    ...
    const isAirportPickup = airportLocations.includes(pickupSelect.value);
    ...
    if (isAirportPickup) {
        ...
        flightNumber.value = '';
        airline.value = '';
        document.getElementById('createRideBaggageClaim').checked = false;
    }
}

// Submit create ride form
function submitCreateRide(event) {
    event.preventDefault();
    ...
    if (!data.name || !data.email || !data.phone || !data.pickup || !data.destination || !data.date || !data.time || !data.vehicleType || !data.passengers) {
        alert('Please fill in all required fields');
        return;
    }
    
    // Check if airport pickup requires flight info
    const airportLocations = [
        'John Wayne Airport (SNA)',
        'Los Angeles Airport (LAX)',
        'Long Beach Airport (LGB)',
    ...
    if (airportLocations.includes(data.pickup)) {
        if (!data.flightNumber || !data.airline) {
            alert('Flight number and airline are required for airport pickups');
            return;
        }
    }
    ...
    try {
        ...
        if (typeof addRide !== 'undefined') {
            ...
            // Switch to rides view and refresh
            switchView('rides');
            loadRides();
            
            alert('Ride created successfully!');
        } else {
            throw new Error('Ride creation system not available');
        }
    } catch (error) {
        console.error('Error creating ride:', error);
        alert(error.message || 'An error occurred while creating the ride. Please try again.');
    }
}

// Block a customer
function blockCustomer(customerId) {
    if (!confirm('Are you sure you want to block this customer? They will not be able to make new reservations using their phone number or email.')) {
        return;
    }
    ...
    saveCustomerArchive(customers);
    loadArchive();
    
    alert('Customer has been blocked. They will not be able to make new reservations.');
}

// Unblock a customer
function unblockCustomer(customerId) {
    if (!confirm('Are you sure you want to unblock this customer? They will be able to make new reservations again.')) {
        return;
    }
    ...
    customer.unblockedAt = new Date().toISOString();
    customer.lastUpdated = new Date().toISOString();
    
    saveCustomerArchive(customers);
    loadArchive();
    
    alert('Customer has been unblocked. They can now make new reservations.');
}
...
function deleteCustomer(customerId) {
    ...
    saveCustomerArchive(filteredCustomers);
    loadArchive();
...
window.importRidesToArchive = importRidesToArchive;
window.switchCustomerFilter = switchCustomerFilter;
...
window.createRideForCustomer = createRideForCustomer;
window.closeCreateRideModal = closeCreateRideModal;
window.handleCreateRideLocationChange = handleCreateRideLocationChange;
window.submitCreateRide = submitCreateRide;
        ...
        const date = new Date(parseInt(dateParts[0]), parseInt(dateParts[1]) - 1, parseInt(dateParts[2])).toLocaleDateString('en-US', { 
            ...
            const serviceFee = driverTotal * 0.15;
...
async function loadSurveys() {
    ...
    try {
        ...
        container.innerHTML = surveys.map(survey => {
            const completedDate = survey.completedAt ? new Date(survey.completedAt).toLocaleDateString('en-US', {
                month: 'short',
                day: 'numeric',
                year: 'numeric',
                hour: 'numeric',
