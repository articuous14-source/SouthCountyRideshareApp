...
function addRide(rideData) {
    ...
    if (isCustomerBlocked(rideData)) {
        throw new Error('This customer has been blocked and cannot make new reservations. Please contact support if you believe this is an error.');
    }
    ...
    // Add customer to archive
    addCustomerToArchive(rideData);
...
// Initialize customer archive array if not exists
function initializeCustomerArchive() {
    if (!localStorage.getItem('customerArchive')) {
        localStorage.setItem('customerArchive', JSON.stringify([]));
    }
}

// Get all customers from archive
function getCustomerArchive() {
    initializeCustomerArchive();
    return JSON.parse(localStorage.getItem('customerArchive')) || [];
}

// Save customer archive
function saveCustomerArchive(customers) {
    localStorage.setItem('customerArchive', JSON.stringify(customers));
}

// Normalize strings for comparison (remove spaces, convert to lowercase)
...
// Check if two customers are the same person
function isSameCustomer(customer1, customer2) {
    // Compare normalized name
    const name1 = normalizeString(customer1.name);
    const name2 = normalizeString(customer2.name);
    
    // Compare normalized email
    const email1 = normalizeString(customer1.email);
    const email2 = normalizeString(customer2.email);
    
    // Compare normalized phone
    const phone1 = normalizePhone(customer1.phone);
    const phone2 = normalizePhone(customer2.phone);
    
    // Compare home addresses if both exist
    const address1 = customer1.homeAddress ? normalizeString(
        `${customer1.homeAddress.street || ''} ${customer1.homeAddress.city || ''} ${customer1.homeAddress.state || ''} ${customer1.homeAddress.zip || ''}`
    ) : '';
    const address2 = customer2.homeAddress ? normalizeString(
        `${customer2.homeAddress.street || ''} ${customer2.homeAddress.city || ''} ${customer2.homeAddress.state || ''} ${customer2.homeAddress.zip || ''}`
    ) : '';
    
    // Match if name AND (email OR phone) match
    // OR if name AND home address match (for same household)
    // OR if email AND phone match (even if name is different - might be typo)
    const nameMatch = name1 === name2 && name1 !== '';
    const emailMatch = email1 === email2 && email1 !== '';
    const phoneMatch = phone1 === phone2 && phone1 !== '';
    const addressMatch = address1 === address2 && address1 !== '';
...
// Add or update customer in archive
function addCustomerToArchive(rideData) {
    ...
    // Extract home address if provided (could be from pickup or destination)
    if (rideData.pickup === 'Home' && rideData.pickupAddress) {
        customerInfo.homeAddress = {
            street: rideData.pickupAddress.street || '',
            city: rideData.pickupAddress.city || '',
            state: rideData.pickupAddress.state || '',
            zip: rideData.pickupAddress.zip || ''
        };
    } else if (rideData.destination === 'Home' && rideData.destinationAddress) {
        customerInfo.homeAddress = {
            street: rideData.destinationAddress.street || '',
            city: rideData.destinationAddress.city || '',
            state: rideData.destinationAddress.state || '',
            zip: rideData.destinationAddress.zip || ''
        };
    }
    ...
    for (let i = 0; i < customers.length; i++) {
        if (isSameCustomer(customers[i], customerInfo)) {
    ...
    if (existingCustomer) {
        ...
        existingCustomer.rideCount = (existingCustomer.rideCount || 1) + 1;
        existingCustomer.lastRideDate = rideData.date;
        existingCustomer.lastUpdated = new Date().toISOString();
        
        // Update home address if we have a new one and didn't have one before
        if (!existingCustomer.homeAddress && customerInfo.homeAddress) {
            existingCustomer.homeAddress = customerInfo.homeAddress;
        }
        
        // Merge home addresses if both exist (keep the most complete one)
        if (existingCustomer.homeAddress && customerInfo.homeAddress) {
            const existingComplete = Object.values(existingCustomer.homeAddress).filter(v => v).length;
            const newComplete = Object.values(customerInfo.homeAddress).filter(v => v).length;
            if (newComplete > existingComplete) {
                existingCustomer.homeAddress = customerInfo.homeAddress;
            }
        }
        
        customers[existingIndex] = existingCustomer;
    } else {
        // Add new customer
        const newCustomer = {
            id: Date.now().toString(),
...
function importRidesToCustomerArchive() {
    const rides = getRides();
    let imported = 0;
    let updated = 0;
    ...
    rides.forEach(ride => {
        ...
        // Extract home address if provided
        if (ride.pickup === 'Home' && ride.pickupAddress) {
            customerInfo.homeAddress = {
                street: ride.pickupAddress.street || '',
                city: ride.pickupAddress.city || '',
                state: ride.pickupAddress.state || '',
                zip: ride.pickupAddress.zip || ''
            };
        } else if (ride.destination === 'Home' && ride.destinationAddress) {
            customerInfo.homeAddress = {
                street: ride.destinationAddress.street || '',
                city: ride.destinationAddress.city || '',
                state: ride.destinationAddress.state || '',
        ...
        if (existingCustomer) {
            // Update existing customer - increment ride count
            existingCustomer.rideCount = (existingCustomer.rideCount || 1) + 1;
            
            // Update dates if this ride is earlier or later
            if (ride.date) {
                if (!existingCustomer.firstRideDate || ride.date < existingCustomer.firstRideDate) {
                    existingCustomer.firstRideDate = ride.date;
                }
                if (!existingCustomer.lastRideDate || ride.date > existingCustomer.lastRideDate) {
                    existingCustomer.lastRideDate = ride.date;
                }
            }
...
// Check if customer is blocked
function isCustomerBlocked(rideData) {
    const customers = getCustomerArchive();
    const normalizedEmail = (rideData.email || '').replace(/\s+/g, '').toLowerCase().trim();
    const normalizedPhone = (rideData.phone || '').replace(/\D/g, '');
    
    return customers.some(customer => {
        if (!customer.blocked) return false;
        
        const customerEmail = (customer.email || '').replace(/\s+/g, '').toLowerCase().trim();
        const customerPhone = (customer.phone || '').replace(/\D/g, '');
        
        return (normalizedEmail === customerEmail && normalizedEmail !== '') ||
               (normalizedPhone === customerPhone && normalizedPhone !== '');
    });
}

// Expose to global scope
window.getCustomerArchive = getCustomerArchive;
window.importRidesToCustomerArchive = importRidesToCustomerArchive;
window.isCustomerBlocked = isCustomerBlocked;
...
if (typeof window !== 'undefined') {
    ...
    // Initialize announcements (if function exists)
    if (typeof initializeAnnouncements === 'function') {
        initializeAnnouncements();
    }
    
    // Initialize invites (if function exists)
    if (typeof initializeInvites === 'function') {
        initializeInvites();
    }
