...
// Get current driver or use default
let currentDriver = getCurrentDriver();
if (!currentDriver) {
    // Use first driver as default if not logged in
...
function applyFilter(rides) {
    ...
    if (filterValue === 'pending') {
    ...
    } else if (filterValue === 'needs-confirmation') {
        // Only show rides that need confirmation (accepted, within 48 hours, not confirmed)
        if (!currentDriver) {
            console.warn('No current driver - cannot filter to needs confirmation');
            return [];
        }
        ...
        console.log(`Filtered: ${filtered.length} rides needing confirmation for driver ${currentDriver.username} out of ${rides.length} total`);
    }
    
    // Apply month filter (only in calendar view)
...
// View unconfirmed rides (switch to list view with needs-confirmation filter)
function viewUnconfirmedRides() {
    // Set filter to needs-confirmation
    const rideFilter = document.getElementById('rideFilter');
    if (rideFilter) {
        rideFilter.value = 'needs-confirmation';
    }
...
document.addEventListener('DOMContentLoaded', function() {
    // Set driver name after DOM is loaded
    if (currentDriver) {
...
async function loadProfile() {
    ...
    // Refresh driver data from API if available to get latest serviceFee
    if (typeof API !== 'undefined' && API.auth) {
        try {
            const user = await API.auth.getCurrentUser();
            if (user && user.user) {
                // Update currentDriver with latest data including serviceFee
                currentDriver = { ...currentDriver, ...user.user };
            }
        } catch (error) {
    ...
    // Load driver info - try currentDriver first (which may have serviceFee), then fallback to local storage
    let driver = currentDriver;
    if (!driver.serviceFee) {
        const drivers = getDrivers();
        const driverFromList = drivers.find(d => d.username === currentDriver.username);
        if (driverFromList && driverFromList.serviceFee !== undefined) {
            driver = { ...driver, serviceFee: driverFromList.serviceFee };
        }
    }
    ...
    try {
        ...
        if (typeof API !== 'undefined' && API.rides) {
            ...
            if (assignSelf && currentDriver) {
                // Get ride ID from response (could be response.ride.id or response.id)
            ...
            resetAddRideForm();
        } else if (typeof addRide !== 'undefined') {
            ...
            if (assignSelf && currentDriver) {
                const rides = getRides();
                const newRide = rides[rides.length - 1];
                if (newRide) {
                    acceptRide(newRide.id, currentDriver);
                }
            }
