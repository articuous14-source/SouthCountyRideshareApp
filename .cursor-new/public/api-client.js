...
async function apiCall(endpoint, options = {}) {
    ...
    if (window.location.protocol === 'file:') {
        const error = new Error('API calls are not available when opening files directly. Please use a web server (e.g., http://localhost:3000)');
    ...
    try {
        ...
        // Don't log CORS errors when using file:// protocol (expected behavior)
        if (error.message && !error.message.includes('CORS') && !error.isFileProtocol) {
